...
const cambiarEst = () => {
    ...
    $.ajax({
        ...
        for(let i = 0; i < response.data.length;i++){
            document.getElementById("Estado").innerHTML+=`<option value="${response.data[i].idEstadoDCDU}">${response.data[i].estado}</option>`;
        }
...
const cambiarEstadoModificado = (carrera) => {
    ...
    $.ajax({
        ...
        for(let i = 0; i < response.data.length;i++){
            if(carrera.idEstadoDCDU==response.data[i].idEstadoDCDU){
                document.getElementById("EstadoModif").innerHTML+=`<option value="${response.data[i].idEstadoDCDU}" selected>${response.data[i].estado}</option>`;
            }else{
                document.getElementById("EstadoModif").innerHTML+=`<option value="${response.data[i].idEstadoDCDU}">${response.data[i].estado}</option>`;
            }
        }
