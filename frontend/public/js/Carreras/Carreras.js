const verificarCamposRegistro = () => {
    ...
    let Dp = { valorEtiqueta: Departamento, id: 'Departamento', type: 'select' };
    let Es = { valorEtiqueta: Estado, id: 'Estado', type: 'select' };
...
const cambiarDepa = () => {
    ...
    $.ajax({
        ...
        for(let i = 0; i < response.length;i++){
            document.getElementById("Departamento").innerHTML+=`<option value="${response[i].idDepartamento}">${response[i].nombreDepartamento}</option>`;
        }
...
const cambiarEst = () => {
    ...
    $.ajax({
        ...
        for(let i = 0; i < response.length;i++){
            document.getElementById("Estado").innerHTML+=`<option value="${response[i].idEstadoDCDU}">${response[i].estado}</option>`;
        }
...
const cambiarDepa2 = () => {
    ...
    $.ajax({
        ...
        for(let i = 0; i < response.length;i++){
            document.getElementById("Departamento2").innerHTML+=`<option value="${response[i].idDepartamento}">${response[i].nombreDepartamento}</option>`;
        }
...
const cambiarDepaModificado = (carrera) => {
    ...
    $.ajax({
        ...
        for(let i = 0; i < response.length;i++){
            if(carrera.idDepartamento==response[i].idDepartamento){
                document.getElementById("DepartamentoModif").innerHTML+=`<option value="${response[i].idDepartamento}" selected>${response[i].nombreDepartamento}</option>`;
            }else{
                document.getElementById("DepartamentoModif").innerHTML+=`<option value="${response[i].idDepartamento}">${response[i].nombreDepartamento}</option>`;
            }
        }
...
const cambiarEstadoModificado = (carrera) => {
    ...
    $.ajax({
        ...
        for(let i = 0; i < response.length;i++){
            if(carrera.idEstadoDCDU==response[i].idEstadoDCDU){
                document.getElementById("EstadoModif").innerHTML+=`<option value="${response[i].idEstadoDCDU}" selected>${response[i].estado}</option>`;
            }else{
                document.getElementById("EstadoModif").innerHTML+=`<option value="${response[i].idEstadoDCDU}">${response[i].estado}</option>`;
            }
        }
...
const cambiarCarreraModif = () => {
    ...
    $.ajax({
        ...
        $("#Carrera2").val(response[0].carrera).trigger("change");
        $("#Abreviatura2").val(response[0].abrev).trigger("change");
        cambiarDepaModificado(response[0]);
        cambiarEstadoModificado(response[0]);
    }).error(function(error) {
       console.warn(error); 
    });
};
const cambiarDepaModif = () => {
    ...
    $.ajax({
        ...
        for(let i = 0; i < response.length;i++){
            document.getElementById("carreraDepa").innerHTML+=`<option value="${response[i].idCarrera}">${response[i].carrera}</option>`;
        }
        $("#botonModif").attr("disabled", true);
    }).error(function(error) {
        console.warn(error);
